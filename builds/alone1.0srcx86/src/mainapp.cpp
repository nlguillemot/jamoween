...
namespace alone
{
	void MainApp::init()
	{
		...
		player_->set_position(static_cast<sf::Vector2f>(background_->point_relative("playerspawn")));
	...
	void MainApp::update(sf::Uint32 dt)
	{
		...
		{
			shadow_time_accumulator_ = 0;
			Shadow::Direction dir = rand()%2 ? Shadow::Left : Shadow::Right;
				...
				if((*it)->started_vanishing())
				{
					if(!player_->awake())
					{
						...
						if(overlay_->scale().x * overlay_->constant("radius") >= window.GetWidth()/2)
						{
							...
							flash_sequencer_.append(std::make_shared<TitleTextChangeSeqItem>(window, messages, 600));
		...
		sf::Vector2f camtopleft = playercenter - sf::Vector2f(view.GetSize().x/2,view.GetSize().y/2);
		sf::FloatRect camrect(camtopleft, view.GetSize());
		camtopleft = CollisionResolver::resolve_inside_bounds(camrect,static_cast<sf::FloatRect>(background_->anim_rect_relative()));
	...
	TitleTextChangeSeqItem::TitleTextChangeSeqItem(sf::Window& window, const std::vector<std::string>& textlist, sf::Uint32 time_between_texts)
		: window_(window),
		textlist_(textlist),
	...
	void TitleTextChangeSeqItem::update(sf::Uint32 dt)
	{
		...
		if(time_accumulator_ >= time_between_texts_)
		{
			...
			{
				window_.SetTitle(textlist_[current_text_]);
			}
		}
	}
}
